...
transform = T.Compose([
    T.Resize(size=518, interpolation=interpolation_mode, antialias=True),
    T.CenterCrop(size=518),
    T.ToTensor(),
...
def process_frame(frame):
    ...
    patch_tokens = model.predict({"image": image_batch})["x_norm_patchtokens"] # 37*37, 384

    fg_pca = PCA(n_components=1)
    reduced_patches = fg_pca.fit_transform(patch_tokens.squeeze())
    print(reduced_patches.shape)
